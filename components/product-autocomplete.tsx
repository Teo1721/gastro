...
export default function ProductAutocomplete({ value, onChange, onSelect, className }: {
  ...
  useEffect(() => {
    ...
    timer.current = window.setTimeout(async () => {
      const q = query.trim()
      const { data } = await supabase.from('inventory_products').select('id,name,unit,last_price,category').ilike('name', `%${q}%`).eq('active', true).limit(8)
  ...
  return (
    <div className={`relative ${className || ''}`} ref={rootRef}>
      <Input value={query} onChange={(e: React.ChangeEvent<HTMLInputElement>) => { setQuery(e.target.value); onChange(e.target.value) }} className="h-9" placeholder="Szukaj produktu..." />
        ...
        <ul className="absolute z-50 bg-white border w-full mt-1 max-h-48 overflow-auto shadow-lg rounded">
          {results.map(r => (
            <li key={r.id} className="px-3 py-2 hover:bg-gray-100 cursor-pointer" onClick={() => { onSelect?.(r); setQuery(r.name); setOpen(false) }}>
              ...
              <div className="text-xs text-slate-500">{r.category || ''} · {r.unit || ''} {r.last_price ? `· ${r.last_price} zł` : ''}</div>
            </li>
          ))}
        </ul>
      )}
